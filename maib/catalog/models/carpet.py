...
class Carpet(models.Model):
    ...
    name = models.CharField("Название ковра", max_length=150, blank=True)
    code = models.CharField("Код модели", max_length=100, blank=True, default='')
    color = models.CharField("Цвет основы", max_length=50)  # только основной цвет
    ...
    def save(self, *args, **kwargs):
        # Автоматическая генерация названия перед сохранением
        if self.collection and self.code and self.color:
            self.name = f"{self.collection.name} {self.code} {self.color}".strip()

        # Генерация основных цветов изображения
        if self.image:
    ...
    def __str__(self):
        return self.name or f"{self.collection.name} ({self.code})"
...
def get_display_name(self):
    return f"{self.collection.name} {self.code} {self.color}".strip()